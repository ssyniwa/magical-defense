    ...
    for x in range(GRID_SIZE):
        with cols[x]:
            ...
            if cell_enemy:
                ...
                hp_text = f"HP:{cell_enemy['hp']},速度:{cell_enemy['speed']},射程:{cell_enemy['range']}"
            elif cell_unit:
                border_color = "#2ecc71"
                ...
                hp_text = f"HP:{cell_unit['hp']},射程:{cell_unit['range']}"
            else:
                border_color = "#aaa"
            ...
            if e["hp"] > 0:
                ...
                if 0 <= spawn_x < GRID_SIZE:
                    ...
                    if not has_unit and not has_enemy:
                        if random.random() < 0.6:  # 60%の確率で毎ターン召喚試行
